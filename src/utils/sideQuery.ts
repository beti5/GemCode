...
import { queryGemini } from '../services/api/gemini.js'
...
export async function sideQuery(opts: SideQueryOptions): Promise<BetaMessage> {
  ...
  if (getAPIProvider() === 'gemini') {
    ...
    const betaMessage = response.assistantMessage.message as BetaMessage & {
      _request_id?: string
    }
    if (response.requestId) {
      betaMessage._request_id = response.requestId
    }
    return betaMessage
  }
